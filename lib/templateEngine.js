...
function buildDocumentBasename(form) {
  const date = form.effectiveDate || 'draft';
  return `Mutual-NDA-${slugifyParty(form.party1.company)}-${slugifyParty(form.party2.company)}-${date}`;
}

function buildDownloadFilename(form) {
  return `${buildDocumentBasename(form)}.md`;
}

function buildPdfFilename(form) {
  return `${buildDocumentBasename(form)}.pdf`;
}
